    ...
    describe('returns false', () => {
      describe('for any not-Number type', () => {
        ...
        describe('different to the global `isNaN` function (specified way before ES6)', () => {
          it('an object gets converted dont to a Number before the check, and returns false therefore', () => {
            const fn = Number.isNaN;
            assert.equal(fn({}), false);
          });
          it('a string gets converted dont to a Number first, and returns false therefore (even though its not `NaN`)', () => {
            const fn = Number.isNaN;
            assert.equal(fn('just a string'), false);
          });
        });
      });
